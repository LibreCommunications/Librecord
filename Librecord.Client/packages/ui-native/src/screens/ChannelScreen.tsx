...
export function ChannelScreen({ navigation, route }: Props) {
    ...
    // Keep a live ref so the realtime handlers (registered once) read current channelId.
    ...
    useEffect(() => {
        ...
        guildMessages.list(channelId, PAGE_SIZE).then((list) => {
            if (cancelled) return;
            // API returns newest first (or oldest first?) — match web by keeping
            // newest at index 0 so inverted FlatList shows newest at the bottom.
    ...
    const loadOlder = useCallback(async () => {
        ...
        try {
            const older = await guildMessages.list(channelId, PAGE_SIZE, oldest.id);
            if (older.length === 0) {
                setHasMore(false);
            } else {
    ...
    // Realtime: new message. Reconcile with optimistic via clientMessageId if present.
    useEffect(() => {
        return onCustomEvent<AppEventMap["guild:message:new"]>("guild:message:new", (d) => {
    ...
    useEffect(() => {
        return onCustomEvent<AppEventMap["guild:message:edited"]>("guild:message:edited", (d) => {
            setMessages((prev) =>
    ...
    useEffect(() => {
        return onCustomEvent<AppEventMap["guild:message:deleted"]>("guild:message:deleted", (d) => {
            setMessages((prev) => prev.filter((m) => m.id !== d.messageId));
        });
    }, []);

    // If the channel itself is deleted, back out.
    useEffect(() => {
    ...
    async function onSend() {
        ...
        try {
            await guildMessages.create(channelId, content, clientMessageId);
            // guild:message:new realtime event will replace the optimistic entry
            // via clientMessageId matching. If the server responded before the
            // event arrives, the realtime handler also dedups by message.id.
        } catch {
            setMessages((prev) =>
                prev.map((m) =>
